...
void TpccOps::payment() {
	...
	HYFLOW_ATOMIC_START {
...

		{
			...
			history->H_DATA = wareHouseName + "    " + districtName;
		}
	}HYFLOW_ATOMIC_END;
}
			...
			if(o_id <= 1) {
...

				{
					...
					for(int orLineNumb=1 ; orLineNumb<=order->O_OL_CNT ; orLineNumb++) {
						if (orLineNumb%(BenchmarkExecutor::getItcpr()) == 0) {
...
void TpccOps::delivery() {
	...
	for (int d_id = 1; d_id <= 10; d_id++) {
		...
		HYFLOW_ATOMIC_START {
			...
			int o_id = 0, ld_o_id = 0, districtNextOId=0;
			{
				HYFLOW_FETCH(districtId, true);
				TpccDistrict* district = (TpccDistrict*) HYFLOW_ON_READ(districtId);

				o_id = district->D_NEXT_O_ID - 1;
				ld_o_id = district->D_LAST_DELV_O_ID;
				LOG_DEBUG("TPCC :Delivery Order Id %d\n", o_id);
				districtNextOId = district->D_NEXT_O_ID;
			}

			for (int ord = o_id; (ord <= o_id) && (ord > ld_o_id); ord--) {
				if (o_id <= 1 || ((districtNextOId - ld_o_id) <= 0)) {
				...
				} else {
					{
						...
						if (ord%(BenchmarkExecutor::getItcpr()) == 0) {
	...
	HYFLOW_ATOMIC_START {
		...
		if (d_next_o_id == 1) {
			...
			for (int o_id = d_next_o_id - 1;(o_id > 1) && ((d_next_o_id - o_id) < 20); o_id--) {
				if (o_id%(BenchmarkExecutor::getItcpr()) == 0) {
...

				{
					int orderOlCnt=0;
					{
						...
						HYFLOW_FETCH(orderId, true);
						TpccOrder* order = (TpccOrder*) HYFLOW_ON_READ(orderId);
						orderOlCnt = order->O_OL_CNT;
					}

					for (int ol_id = 1; ol_id <= orderOlCnt; ol_id++) {
