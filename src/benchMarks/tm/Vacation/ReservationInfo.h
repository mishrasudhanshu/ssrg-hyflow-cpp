...
class ReservationInfoArgs {
public:
	int money;
	std::string id1;
	std::string id2;

	ReservationInfoArgs(int m, std::string i1, std::string i2) {
		money= m;
		id1 = i1;
		id2 = i2;
	}
...
public:
	ReservationInfo() {};
	ReservationInfo(uint64_t amount, const std::string & Id);
	ReservationInfo(uint64_t amount, const std::string & Id, int version);
