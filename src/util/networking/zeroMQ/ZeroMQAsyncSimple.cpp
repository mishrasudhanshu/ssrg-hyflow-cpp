...
int ZMQNetworkAsyncSimple::nodeId = -1;
int ZMQNetworkAsyncSimple::basePort = -1;
int ZMQNetworkAsyncSimple::nodeCount = 0;
std::string* ZMQNetworkAsyncSimple::nodeIPs = NULL;

volatile bool ZMQNetworkAsyncSimple::hyflowShutdown = false;
int ZMQNetworkAsyncSimple::lingerTime = 0;

bool ZMQNetworkAsyncSimple::isInit = false;
int ZMQNetworkAsyncSimple::threadCount = 0;
zmq::socket_t* ZMQNetworkAsyncSimple::nodeInitSocket = NULL;
zmq::context_t* ZMQNetworkAsyncSimple::context=NULL;

std::vector<zmq::socket_t*> ZMQNetworkAsyncSimple::threadRouterSockets;
std::vector<pthread_t> ZMQNetworkAsyncSimple::dealerThreads;
std::vector<int*> ZMQNetworkAsyncSimple::dealerThreadIds;

ZMQNetworkAsyncSimple::ZMQNetworkAsyncSimple() {
	if (!isInit) {
...
void ZMQNetworkAsyncSimple::additionalSync(){
	if (nodeId == 0) {
...
void ZMQNetworkAsyncSimple::networkInit(){
	// Nothing to do
	sleep(2);
}

void ZMQNetworkAsyncSimple::networkShutdown(){
	// Nothing is required to be done, we do clean up in destructor call
}

void ZMQNetworkAsyncSimple::sendMessage(int toNodeId, HyflowMessage & message){
...
void ZMQNetworkAsyncSimple::sendCallbackMessage(int toNodeId, HyflowMessage & message, HyflowMessageFuture & fu){
	// Serialize the message
	std::ostringstream ostream;
	boost::archive::text_oarchive oa(ostream);
	oa << message;
...
bool ZMQNetworkAsyncSimple::defaultHandler(zmq::message_t & msg) {
...
void ZMQNetworkAsyncSimple::callbackHandler(zmq::message_t & msg){
...
void* ZMQNetworkAsyncSimple::dealerExecute(void *param) {
...
void ZMQNetworkAsyncSimple::s_catch_signals() {
...
void ZMQNetworkAsyncSimple::s_signal_handler(int signal_value) {
