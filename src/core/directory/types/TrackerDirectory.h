...
class TrackerDirectory: public vt_dstm::HyflowDirectory {
	static std::map<std::string, int> directory;
	static std::map<std::string, HyflowObject*> local;
...
public:
	...
	void registerObjectLocally(HyflowObject & object, unsigned long long txn);
